...
import { searchMovies, fetchTrending, fetchTopRatedByYear, fetchLatestByCountry } from "@/lib/tmdb";
...
interface HomeClientProps {
  ...
  initialNowPlaying: TMDBMovie[];
  initialTopRated: TMDBMovie[];
  initialLatestVietnam: TMDBMovie[];
}

export function HomeClient({ initialTrending, initialNowPlaying, initialTopRated, initialLatestVietnam }: HomeClientProps) {
  ...
  const [topRatedMovies, setTopRatedMovies] = useState<TMDBMovie[]>(topIMDBMovies as any);
  const [loadingTopRated, setLoadingTopRated] = useState(false);

  // Latest by Country state
  const [latestCountry, setLatestCountry] = useState<"vietnam" | "usa" | "korea" | "japan">("vietnam");
  const [latestMovies, setLatestMovies] = useState<TMDBMovie[]>(initialLatestVietnam);
  ...
  // ── Parse Search Query Parameter từ URL (để chuyển hướng từ trang khác về) ──
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (queryParam) {
        ...
        // Clear query param để không bị kẹt khi reload
        window.history.replaceState({}, document.title, window.location.pathname);
      }
    }
  }, []);

  // ── Load Trending Data khi Tab (Today / This Week) thay đổi ──────────────────
  useEffect(() => {
    // Tránh fetch thừa khi mới render lần đầu
    if (timeWindow === "week" && trendingMovies === initialTrending) return;

    const loadTrending = async () => {
      setLoadingTrending(true);
      try {
        const fetched = await fetchTrending(timeWindow);
  ...
  // ── Load Top Rated Data khi Filter theo năm thay đổi ────────────────────────
  useEffect(() => {
    if (topRatedYear === "all") {
      setTopRatedMovies(topIMDBMovies as any);
      return;
    }
    ...
    const loadTopRated = async () => {
      ...
      try {
        const fetched = await fetchTopRatedByYear(topRatedYear);
        
        // Cần lấy điểm IMDb thực tế cho các phim 2026 này và sắp xếp theo điểm IMDb đó!
        const withImdb = await Promise.all(
          fetched.map(async (m) => {
            const y = m.release_date ? parseInt(m.release_date.substring(0, 4)) : null;
            const data = await getOmdbData(m.id, m.title, y, "movie", String(m.vote_average));
        ...
        // Sắp xếp các phim 2026 theo điểm IMDb thực tế giảm dần
        const sorted = withImdb.sort((a, b) => b.imdbRating - a.imdbRating);
        setTopRatedMovies(sorted);
      } catch (err) {
        console.error("Failed to load top rated by year:", err);
      } finally {
  ...
  // ── Load Latest Movies by Country ──────────────────────────────────────────
  useEffect(() => {
    if (latestCountry === "vietnam" && latestMovies === initialLatestVietnam) return;

    const loadLatest = async () => {
      setLoadingLatest(true);
      try {
        const fetched = await fetchLatestByCountry(latestCountry);
        setLatestMovies(fetched);
      } catch (err) {
        console.error("Failed to load latest movies by country:", err);
      } finally {
        setLoadingLatest(false);
      }
    };
    loadLatest();
  }, [latestCountry, initialLatestVietnam]);

  // ── Realtime Search (TMDB) ─────────────────────────────────────────────────
  useEffect(() => {
    if (!searchQuery.trim()) {
      setSearchResults([]);
      return;
  ...
  // ── Xử lý Trượt ngang Carousel bằng nút ấn ──────────────────────────────────
  const handleScroll = (ref: React.RefObject<HTMLDivElement | null>, direction: "left" | "right") => {
    if (ref.current) {
      const scrollAmount = 600; // Trượt 600px mỗi lần click
  ...
  const filteredNowPlaying = (() => {
    if (nowPlayingWindow === "day") {
      const now = new Date();
      return [...initialNowPlaying].filter((movie) => {
  ...
  const finalNowPlaying = (() => {
    if (nowPlayingWindow === "day" && filteredNowPlaying.length < 5) {
      // Fallback: sort by release date descending (newest first)
      return [...initialNowPlaying].sort((a, b) => {
  ...
  // Chỉ lấy top 10 phim cho mỗi hàng
      ...
      <main className="mx-auto max-w-[1200px] px-4 py-8">
        ...
        {searchQuery.trim() ? (
          ...
          <div className="space-y-12">
              ...
              <div className="relative group/carousel-trending w-full overflow-hidden">
                ...
                >
                  ...
                  {topTrending.map((movie) => (
                    <div
                      key={`trending-${movie.media_type ?? "movie"}-${movie.id}`}
            ...
            {/* ── 2. SECTION NOW PLAYING: Carousel Trượt ngang ── */}
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-5">
                <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">
                  Now Playing
              ...
              <div className="relative group/carousel-nowplaying w-full overflow-hidden">
                ...
                >
                  {topNowPlaying.map((movie) => (
                    <div
                      key={`nowplaying-${movie.media_type ?? "movie"}-${movie.id}`}
                      className="w-[150px] sm:w-[170px] flex-shrink-0"
                    >
                      <MovieCard movie={movie} />
                    </div>
            ...
            {/* ── 3. SECTION TOP IMDB: Carousel Trượt ngang ── */}
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex flex-wrap items-center gap-5">
                  <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">
                ...
                <Link
                  href="/top-250-imdb"
                  className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground hover:text-[#00e054] transition-colors border border-border/60 hover:border-[#00e054]/40 px-3 py-1 rounded-full bg-[#1c2228]/50"
                >
                  More →
                </Link>
              </div>
              ...
              <div className="relative group/carousel-topimdb w-full overflow-hidden">
                ...
                >
                  {topTopRated.map((movie) => (
                    <div
                      key={`toprated-${movie.media_type ?? "movie"}-${movie.id}`}
                      className="w-[150px] sm:w-[170px] flex-shrink-0"
                    >
                      <MovieCard movie={movie} />
                    </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-5">
                ...
                {/* Capsules Toggles: Vietnam / USA / Korea / Japan */}
                <div className="flex items-center rounded-full border border-border/40 p-0.5 bg-[#14181c]/80 backdrop-blur-sm shadow-inner">
                  <button
                    onClick={() => setLatestCountry("vietnam")}
                    className={`px-4 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all duration-200 select-none ${
                      latestCountry === "vietnam"
                        ? "bg-[#0d253f] text-[#01b4e4] font-extrabold shadow"
                        : "text-[#9ab] hover:text-white"
                    }`}
                  >
              ...
              <div className="relative group/carousel-latest w-full overflow-hidden">
                ...
                >
                  {latestMovies.map((movie) => (
                    <div
                      key={`latest-${movie.media_type ?? "movie"}-${movie.id}`}
                      className="w-[150px] sm:w-[170px] flex-shrink-0"
                    >
                      <MovieCard movie={movie} />
                    </div>
