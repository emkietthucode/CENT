...
function normalizeWatchedOn(watchedOn: string): string {
  ...
  if (/^\d{1,2}\s[A-Za-z]{3}\s\d{4}$/.test(watchedOn.trim())) {
    return watchedOn.trim();
  }
  // ISO format: "YYYY-MM-DD"
  const isoMatch = watchedOn.trim().match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (isoMatch) {
    const [, y, m, d] = isoMatch;
    const monthIdx = parseInt(m, 10) - 1;
    return `${parseInt(d, 10)} ${MONTHS_SHORT[monthIdx]} ${y}`;
  }
  // Fallback: try parsing as generic date
  const parsed = new Date(watchedOn);
...
export async function importFromCSV(
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    try {
      ...
      const now = new Date();
      const defaultWatchedOn = `${now.getDate()} ${months[now.getMonth()]} ${now.getFullYear()}`;
      ...
      if (watchedOnIdx !== -1 && row[watchedOnIdx]) {
        ...
        try {
          ...
          if (!isNaN(parsedDate.getTime())) {
            watched_on = `${parsedDate.getDate()} ${months[parsedDate.getMonth()]} ${parsedDate.getFullYear()}`;
          } else {
            watched_on = rawDate;
          }
        } catch {
